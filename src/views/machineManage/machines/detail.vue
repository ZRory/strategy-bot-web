<template>
  ...
  >
    <el-alert
      title="策略关闭时会强制卖出所有仓位，盈亏金额将会显示为负数!"
      type="error"
    >
    </el-alert>
    <!-- 任务列表 -->
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <span class="box-card-header">策略仓位</span>
      </div>
      <!--表格渲染-->
      <el-table
        ref="table"
      ...
      >
        ...
        <el-table-column label="操作" width="150px" align="center" v-if="this.$route.query.share !== 'true'">
          <template slot-scope="scope">
            ...
            >
              ...
              <div style="text-align: right; margin: 0">
                ...
                <el-button :loading="closeLoading" type="primary" size="mini"
...
<script>
import {closePosition, listPlotsHistory, unbinding} from '@/api/monitor/machine'
...
export default {
  ...
  created() {
    this.strategyId = this.$route.query.clientId
    this.queryParam.strategyId = this.$route.query.clientId
    this.queryList()
  },
  destroyed() {
  },
  methods: {
    queryList() {
      ...
      listPlotsHistory(this.queryParam).then(res => {
        this.show = true
